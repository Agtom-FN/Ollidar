...
Current version: **0.9.19** (Android).
...
| | Minimum | Notes |
| --- | --- | --- |
| Android version | Android 10 (API 29) | Built for Android 16 (API 36). The app installs without ARCore, but real scanning needs Google's AR tracking, which needs roughly Android 14+ on a supported device |
| Required hardware | USB host (OTG), OpenGL ES 3.0 GPU | The serial lidars plug in over USB-C; required by the manifest |
| Optional hardware | — | ARCore-capable camera (needed for real scanning), gyroscope (recommended), Ethernet adapter support (Mid-360 only), Bluetooth (RTK rover only), GPS (outdoor georeferencing only) — the app runs without these, feature-gated |
| RAM | Any (auto-tiered) | Rendering auto-tiers to the device — roughly modest phones get a 96 MB point budget, standard 256 MB, flagships 512 MB. Field-validated device: Google Pixel 8 Pro |
...
2. Mount the lidar flat on the back of the phone (zero mark up, cap pointing
   ...
   Ethernet (see the Mid-360 adapter notes below).
...
- **Scanning reliability** — automatic mount re-zero defended against bad
  ...
  for tilted floors, and a run of stability fixes including a crash recorder.
- **Full UI redesign** — a new Simple Mode, the Agtom orange theme and app
  icon, a light theme, and fully responsive layouts down to small phones and
  large text, alongside the rename to **Ollidar** and its llama icon.
- **Height colours + viewer gestures** — real orbit/pan/zoom gestures, a
  measure tool, and a Turbo height-colour ramp by default, with existing
  scans migrated automatically.
- **Mid-360 diagnostics + STL-27L support in validation** — a guided
  Ethernet setup wizard with a hidden developer diagnostics mode, adapter
  buying guidance, and an STL-27L driver moving through bench validation.
- **GitHub feedback and diagnostics** — a Profile screen with Send logs, an
  in-app feedback box, and issue reporting wired closer to GitHub.
- **Welcome animation + posture indicator** — a first-launch welcome
  animation and an in-scan posture indicator, added in the later betas.

Per-version details live on the
[GitHub Releases page](https://github.com/Agtom-FN/Ollidar/releases).
